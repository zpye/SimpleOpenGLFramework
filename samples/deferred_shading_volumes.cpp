...
int main() {
  ...
  std::string g_buffer_vertex_shader = shader_path + "/g_buffer.vs";
  std::string g_buffer_fragment_shader = shader_path + "/g_buffer.fs";
  std::string deferred_shading_vertex_shader =
      shader_path + "/deferred_shading.vs";
  std::string deferred_shading_fragment_shader =
      shader_path + "/deferred_shading.fs";
  std::string deferred_light_box_vertex_shader =
      shader_path + "/deferred_light_box.vs";
  std::string deferred_light_box_fragment_shader =
      shader_path + "/deferred_light_box.fs";
  Shader shaderGeometryPass(g_buffer_vertex_shader.c_str(),
                            g_buffer_fragment_shader.c_str());
  Shader shaderLightingPass(deferred_shading_vertex_shader.c_str(),
                            deferred_shading_fragment_shader.c_str());
  Shader shaderLightBox(deferred_light_box_vertex_shader.c_str(),
                        deferred_light_box_fragment_shader.c_str());
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (unsigned int i = 0; i < lightPositions.size(); ++i) {
      ...
      const float constant =
          1.0;  // note that we don't send this to the shader, we assume it is
