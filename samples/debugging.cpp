...
int main() {
  ...
  glfwWindowHint(GLFW_OPENGL_DEBUG_CONTEXT,
                 GL_TRUE);  // comment this line in a release build!
  ...
  std::string debugging_vertex_shader = shader_path + "/debugging.vs";
  std::string debugging_fragment_shader = shader_path + "/debugging.fs";
  Shader shader(debugging_vertex_shader.c_str(),
                debugging_fragment_shader.c_str());
  ...
  unsigned char *data = stbi_load((resource_path + "/wood.png").c_str(), &width,
                                  &height, &nrComponents, 0);
  if (data) {
    glTexImage2D(GL_FRAMEBUFFER, 0, GL_RGB, width, height, 0, GL_RGB,
  ...
  glm::mat4 projection = glm::perspective(
      glm::radians(45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 10.0f);
