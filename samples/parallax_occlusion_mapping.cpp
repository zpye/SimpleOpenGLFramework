...
int main() {
  ...
  std::string parallax_mapping_vertex_shader =
      shader_path + "/parallax_mapping.vs";
  std::string parallax_mapping_fragment_shader =
      shader_path + "/parallax_mapping.fs";
  Shader shader(parallax_mapping_vertex_shader.c_str(),
                parallax_mapping_fragment_shader.c_str());

  // load textures
  std::string resource_path(RESOURCE_PATH);
  unsigned int diffuseMap =
      loadTexture((resource_path + "/bricks2.jpg").c_str());
  unsigned int normalMap = loadTexture(
      (resource_path + "/bricks2_normal.jpg").c_str());
  unsigned int heightMap = loadTexture(
      (resource_path + "/bricks2_disp.jpg").c_str());
  /*unsigned int diffuseMap =
 ...
 loadTexture(FileSystem::getPath("resources/textures/toy_box_disp.png").c_str());*/
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // rotate the quad to show parallax
