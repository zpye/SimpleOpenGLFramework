...
int main() {
  ...
  std::string anti_aliasing_vertex_shader = shader_path + "/anti_aliasing.vs";
  std::string anti_aliasing_fragment_shader = shader_path + "/anti_aliasing.fs";
  std::string aa_post_vertex_shader = shader_path + "/aa_post.vs";
  std::string aa_post_fragment_shader = shader_path + "/aa_post.fs";
  Shader shader(anti_aliasing_vertex_shader.c_str(),
                anti_aliasing_fragment_shader.c_str());
  Shader screenShader(aa_post_vertex_shader.c_str(),
                      aa_post_fragment_shader.c_str());
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glBindTexture(GL_TEXTURE_2D,
                  screenTexture);  // use the now resolved color attachment as
                                   // the quad's texture
