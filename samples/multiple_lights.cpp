...
int main() {
  ...
  std::string lighting_vertex_shader = shader_path + "/multiple_lights.vs";
  std::string lighting_fragment_shader = shader_path + "/multiple_lights.fs";
  std::string lamp_vertex_shader = shader_path + "/lamp.vs";
  std::string lamp_fragment_shader = shader_path + "/lamp.fs";
  Shader lightingShader(lighting_vertex_shader.c_str(),
                        lighting_fragment_shader.c_str());
  Shader lampShader(lamp_vertex_shader.c_str(), lamp_fragment_shader.c_str());
  ...
  std::string img1_path = resource_path + "/container2.png";
  std::string img2_path = resource_path + "/container2_specular.png";
  unsigned int diffuseMap = loadTexture(img1_path.c_str());
  unsigned int specularMap = loadTexture(img2_path.c_str());
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    */
    ...
    for (unsigned int i = 0; i < 4; i++) {
      ...
      lampShader.setVec3("ourColor", pointLightColors[i]);
      glDrawArrays(GL_TRIANGLES, 0, 36);
    }
